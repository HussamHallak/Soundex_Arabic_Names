...
libindic_results_csv = open('libindic_results.csv', 'w', newline='')
libindic_writer = csv.DictWriter(libindic_results_csv, fieldnames=fieldnames)
libindic_writer.writeheader()

jellyfish_match_rating_results_csv = open('jellyfish_match_rating_results.csv', 'w', newline='')
...
jellyfish_soundex_results_csv = open('jellyfish_soundex_results.csv', 'w', newline='')
...
fuzzy_nysiis_results_csv = open('fuzzy_nysiis_results.csv', 'w', newline='')
...
levenshtine_results_csv = open('levenshtine_results.csv', 'w', newline='')
levenshtine_writer = csv.DictWriter(levenshtine_results_csv, fieldnames=fieldnames)
levenshtine_writer.writeheader()
...
def jellyfish_match_rating_isEqual(name1, name2):
	if jellyfish.match_rating_codex(name1) == jellyfish.match_rating_codex(name2):
		return 1
	else:
		return 0
...
def fuzzy_nysiis_isEqual(name1, name2):
	if fuzzy.nysiis(name1) == fuzzy.nysiis(name2):
		return 1
	else:
		return 0
...
def fuzzy_dmeta_isEqual(name1, name2):
	...
	if dmeta(name1)[0] == dmeta(name2)[0] or dmeta(name1)[0] == dmeta(name2)[1] or dmeta(name1)[1] == dmeta(name2)[0]:
		return 1
	else:
		return 0

def levenshtine_isEqual(str1, str2):
...
for name1, name2 in itertools.combinations(names, 2):
	if name1 != name2:
		...
		levenshtine_writer.writerow({'Name1': name1, 'Name2': name2, 'Output': levenshtine_isEqual(name1, name2)})
