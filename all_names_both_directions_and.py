...
libindic_results_csv = open('2D_and_libindic_results.csv', 'w', newline='')
libindic_writer = csv.DictWriter(libindic_results_csv, fieldnames=fieldnames)
libindic_writer.writeheader()

jellyfish_match_rating_results_csv = open('2D_and_jellyfish_match_rating_results.csv', 'w', newline='')
...
jellyfish_soundex_results_csv = open('2D_and_jellyfish_soundex_results.csv', 'w', newline='')
...
fuzzy_nysiis_results_csv = open('2D_and_fuzzy_nysiis_results.csv', 'w', newline='')
fuzzy_nysiis_writer = csv.DictWriter(fuzzy_nysiis_results_csv, fieldnames=fieldnames)
fuzzy_nysiis_writer.writeheader()

fuzzy_dmeta_results_csv = open('2D_and_fuzzy_dmeta_results.csv', 'w', newline='')
...
def libindic_isEqual(name1, name2):
	instance = Soundex()
	if instance.soundex(name1) == instance.soundex(name2) and instance.soundex(reverse(name1)) == instance.soundex(reverse(name2)):
		return 1
	else:
		return 0

def jellyfish_match_rating_isEqual(name1, name2):
	if jellyfish.match_rating_codex(name1) == jellyfish.match_rating_codex(name2) and jellyfish.match_rating_codex(reverse(name1)) == jellyfish.match_rating_codex(reverse(name2)):
		return 1
	else:
		return 0
		
def jellyfish_soundex_isEqual(name1, name2):
	if jellyfish.soundex(name1) == jellyfish.soundex(name2) and jellyfish.soundex(reverse(name1)) == jellyfish.soundex(reverse(name2)):
		return 1
	else:
		return 0
		
def fuzzy_nysiis_isEqual(name1, name2):
	if fuzzy.nysiis(name1) == fuzzy.nysiis(name2) and fuzzy.nysiis(reverse(name1)) == fuzzy.nysiis(reverse(name2)):
		return 1
	else:
		return 0
...
def fuzzy_dmeta_isEqual(name1, name2):
	...
	if (dmeta(name1)[0] == dmeta(name2)[0] or dmeta(name1)[0] == dmeta(name2)[1] or dmeta(name1)[1] == dmeta(name2)[0]) and (dmeta(reverse(name1))[0] == dmeta(reverse(name2))[0] or dmeta(reverse(name1))[0] == dmeta(reverse(name2))[1] or dmeta(reverse(name1))[1] == dmeta(reverse(name2))[0]):
		return 1
	else:
		return 0
